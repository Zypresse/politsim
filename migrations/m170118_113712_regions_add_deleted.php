<?php

use yii\db\Migration;

class m170118_113712_regions_add_deleted extends Migration
{
    public function safeUp()
    {
        $this->renameTable('regions', 'tmpRegions');
        $this->createTable('regions', [
            'id' => 'INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL',
            'stateId' => 'UNSIGNED INTEGER REFERENCES states(id) DEFAULT NULL',
            'cityId' => 'UNSIGNED INTEGER REFERENCES cities(id) DEFAULT NULL',
            'name' => 'VARCHAR(255) NOT NULL',
            'nameShort' => 'VARCHAR(6) NOT NULL',
            'flag' => 'TEXT DEFAULT NULL',
            'anthem' => 'TEXT DEFAULT NULL',
            'population' => 'UNSIGNED INTEGER NOT NULL DEFAULT 0',
            
            'classes' => 'TEXT DEFAULT NULL',
            'nations' => 'TEXT DEFAULT NULL',
            'ideologies' => 'TEXT DEFAULT NULL',
            'religions' => 'TEXT DEFAULT NULL',
            'genders' => 'TEXT DEFAULT NULL',
            'ages' => 'TEXT DEFAULT NULL',
            'contentment' => 'UNSIGNED REAL NOT NULL DEFAULT 0',
            'agression' => 'UNSIGNED REAL NOT NULL DEFAULT 0',
            'consciousness' => 'UNSIGNED REAL NOT NULL DEFAULT 0',
            
            'dateCreated' => 'UNSIGNED INTEGER DEFAULT NULL',
            'dateDeleted' => 'UNSIGNED INTEGER DEFAULT NULL',
            'implodedTo' => 'UNSIGNED INTEGER REFERENCES regions(id) DEFAULT NULL',
            
            'usersCount' => 'UNSIGNED INTEGER NOT NULL DEFAULT 0',
            'usersFame' => 'UNSIGNED INTEGER NOT NULL DEFAULT 0',
            'utr' => 'UNSIGNED INTEGER REFERENCES utr(id) DEFAULT NULL'
        ]);
        
        $this->createIndex('regionState', 'regions', ['stateId']);
        
        $this->execute("
            INSERT INTO regions 
            (id,stateId,cityId,name,nameShort,flag,anthem,population,classes,nations,ideologies,religions,genders,ages,contentment,agression,consciousness,usersCount,usersFame,utr)
            SELECT id,stateId,cityId,name,nameShort,flag,anthem,population,classes,nations,ideologies,religions,genders,ages,contentment,agression,consciousness,usersCount,usersFame,utr FROM `tmpRegions`
        ");
        $this->dropTable('tmpRegions');
    }

    public function safeDown()
    {
        $this->renameTable('regions', 'tmpRegions');
        $this->createTable('regions', [
            'id' => 'INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL',
            'stateId' => 'UNSIGNED INTEGER REFERENCES states(id) DEFAULT NULL',
            'cityId' => 'UNSIGNED INTEGER REFERENCES cities(id) DEFAULT NULL',
            'name' => 'VARCHAR(255) NOT NULL',
            'nameShort' => 'VARCHAR(6) NOT NULL',
            'flag' => 'TEXT DEFAULT NULL',
            'anthem' => 'TEXT DEFAULT NULL',
            'population' => 'UNSIGNED INTEGER NOT NULL DEFAULT 0',
            
            'classes' => 'TEXT DEFAULT NULL',
            'nations' => 'TEXT DEFAULT NULL',
            'ideologies' => 'TEXT DEFAULT NULL',
            'religions' => 'TEXT DEFAULT NULL',
            'genders' => 'TEXT DEFAULT NULL',
            'ages' => 'TEXT DEFAULT NULL',
            'contentment' => 'UNSIGNED REAL NOT NULL DEFAULT 0',
            'agression' => 'UNSIGNED REAL NOT NULL DEFAULT 0',
            'consciousness' => 'UNSIGNED REAL NOT NULL DEFAULT 0',
            
            'usersCount' => 'UNSIGNED INTEGER NOT NULL DEFAULT 0',
            'usersFame' => 'UNSIGNED INTEGER NOT NULL DEFAULT 0',
            'utr' => 'UNSIGNED INTEGER REFERENCES utr(id) DEFAULT NULL'
        ]);
        
        $this->execute("
            INSERT INTO regions 
            (id,stateId,cityId,name,nameShort,flag,anthem,population,classes,nations,ideologies,religions,genders,ages,contentment,agression,consciousness,usersCount,usersFame,utr)
            SELECT id,stateId,cityId,name,nameShort,flag,anthem,population,classes,nations,ideologies,religions,genders,ages,contentment,agression,consciousness,usersCount,usersFame,utr FROM `tmpRegions`
        ");
        $this->dropTable('tmpRegions');
    }
}
