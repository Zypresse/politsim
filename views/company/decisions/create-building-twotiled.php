...
?>
<?= $form->field($model, 'dataArray[protoId]')->dropDownList(BuildingTwotiledProto::getList())->label(Yii::t('app', 'Building type')) ?>
...
<div class="form-group">
    <label class="control-label" ><?=Yii::t('app', 'Start location')?></label>
...
<div class="form-group">
    <label class="control-label" ><?=Yii::t('app', 'Finish location')?></label>
    <?= Html::dropDownList('selectedState2Id', $company->stateId, ArrayHelper::map(State::find()->where(['dateDeleted' => null])->orderBy(['name' => SORT_ASC])->all(), 'id', 'name'), ['id' => 'selected-state2-id', 'class' => 'form-control']) ?>
    <?= Html::dropDownList('selectedRegion2Id', null, ArrayHelper::map($company->state->regions, 'id', 'name'), ['id' => 'selected-region2-id', 'class' => 'form-control']) ?>
</div>
<div id="build-map"></div>
<?= $form->field($model, 'dataArray[tileId]', ['labelOptions' => ['class' => 'hide']])->hiddenInput() ?>
<?= $form->field($model, 'dataArray[tile2Id]', ['labelOptions' => ['class' => 'hide']])->hiddenInput() ?>
