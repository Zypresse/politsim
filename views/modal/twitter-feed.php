...
foreach ($tweets as $i => $tweet) { ?>
 ...
 <p class="tweet-footer"><?php if ($tweet->uid !== $viewer_id && $tweet->original !== $viewer_id) { ?><a href="#" class="btn btn-default btn-xs repost" title="Репост" data-id="<?=$tweet->id?>"><i class="fa fa-repeat"></i></a><?php } else { ?><i class="fa fa-repeat"></i><?php } ?> <?=MyHtmlHelper::formateNumberword($tweet->retweets,'репостов','репост','репоста')?></p>
 </div>
<?php } ?>