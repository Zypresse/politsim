...
<p><input style="margin-top: 10px; display:inline-block;float: left; margin-right:20px" class="elect_vote_radio" type="radio" name="elect_vote" value="<?=$elect_request->id?>" data-child="0"> &nbsp; <a href="#" onclick="$('.modal-backdrop').hide();load_page('party_info',{'id':<?=$elect_request->party_id?>})"><strong><?=htmlspecialchars($elect_request->party->name)?></strong></a></p>
