...
<?php $form->begin([
    ...
    'action' => Url::to(['building/vacancy-edit-form']),
    'enableClientValidation' => true,
    'enableAjaxValidation' => true,
    'validationUrl' => Url::to(['building/vacancy-edit-form', 'id' => $building->id, 'vacancyId' => $model->id])
]) ?>
...
<script type="text/javascript">
    ...
    $form.on('submit', function() {
        if ($form.yiiActiveForm('data').validated) {
            json_request('building/vacancy-edit?id=<?= $building->id ?>&vacancyId=<?= $model->id ?>', $form.serializeObject(), false, false, false, 'POST');
        }
        return false;
    });
